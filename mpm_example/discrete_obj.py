    ...
    for i in range(grid_num):
        ...
        for j in range(grid_num):
            for k in range(grid_num):
                ...
                min_index = 0
                # 网格节点位置
                node_pos = np.array([i * dx, j * dx, k * dx])
                nearest_point = np.array([.0, .0, .0])
                for triangle in range(int(indexes.shape[0] / 3)):
                    A = vertices[indexes[triangle * 3]]
                    B = vertices[indexes[triangle * 3 + 1]]
                    C = vertices[indexes[triangle * 3 + 2]]
                    distance, min_point = point_triangle_distance_min(A, B, C, node_pos)
                    if distance < min_dis:
                        min_dis = distance
                        nearest_point = min_point
                        min_index = triangle * 3
                # 判断正负
                n1 = normals[normal_indices[min_index]]
                n2 = normals[normal_indices[min_index + 1]]
                n3 = normals[normal_indices[min_index + 2]]
                n = (n1 + n2 + n3) / 3
                sign = np.dot(n, (node_pos - nearest_point))
                if sign <= 0:
                    result.append(nearest_point)
    print(result)
    np.save("cirque.npy", result)
    return vertices, indexes, normals, normal_indices

banana = np.load('cirque.npy')
print(banana.shape)
# read_obj("cirque.obj")
